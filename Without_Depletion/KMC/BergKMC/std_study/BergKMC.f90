...
real*8 :: t(n_sim,n_st),av_h(n_sim), av_sim(n_sim), sd_sim(n_sim)
...
allocate(t_smpl(n_sim,nsmpl))
allocate(N_smpl(n_sim,nsmpl))
...
do j=1,n_sim 	
    ...
    do k=1,nsmpl
      ...
      sum_av = sum_av + N_smpl(j,k)
      
    end do
      
    av_sim(j)=sum_av/real(nsmpl)
    
    sum_sd=0.0d0
    do k=1,nsmpl
      
      sum_sd = sum_sd + (N_smpl(j,k)-av_sim(j))**2.0d0      
    end do
      
    sd_sim(j) = dsqrt(sum_sd/real(nsmpl))
