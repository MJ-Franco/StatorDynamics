...
subroutine KMC_Traces(rkon,rkoff,rn0,n_v,n_sim,n_st,N_smpl,t_smpl,ntt)
...
integer :: N(n_sim,n_st), ntt(n_sim)
...
do j=1,n_sim ! Begin of one story
  ...
  do l = 1,n_st-1
    ...
    allocate(t_smpl(n_sim,ntt(j)))
    allocate(N_smpl(n_sim,ntt(j)))
