...
subroutine KMC_NBerg(rkon,rkt,rkl,rkoff,rn0,n_v,n_sim,n_st,N_smpl, &
t_smpl,ntt)
...
do j=1,n_sim ! Begin of one story
  ...
  do l = 1,n_st-1
    ...
    if (N(j,l)==0) then
      ...
      rup = rkon*(1.0d0+A*(1.0d0-dexp(-B*N(j,l)))* &
      (1.0d0-dexp(-C/N(j,l))))*(real(n_v)-N(j,l))
      rdw=rkoff/(1.0d0 + rkt/rkl + rkoff/rkl)*real(N(j,l))
    end if
